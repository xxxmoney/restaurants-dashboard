...
export const MENU_PROMPTS = {
    [restaurantEnum.CINKY_LINKY]: `Using the provided screenshot of menu from the restaurant U Sisku, importantly take into account that current year is ${DateTime.now().year} so use it for dates for menus, return the menus for each day, if there are none, return empty, stick strictly to what is only visible in the screenshot`,
}
